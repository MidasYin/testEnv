...
class Test_block_index_until_exist_data:
    @classmethod
    def setup_class(cls):
        pass
    ...
    def test_block_index_until_exist_data(self):
        ...
        if reportIds != []:
            for reportId in reportIds:
                ...
                url = "/ecg/block_index_until_exist_data?report_id=" + str(reportId[1]) + "&block_index=0"
                result = Request.execute("config_reviewDr.xlsx", "ecg", "block_index_until_exist_data", url)
                log.info("返回结果为：")
                log.info(result)
