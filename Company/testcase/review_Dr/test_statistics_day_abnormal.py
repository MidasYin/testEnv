...
class Test_statistics_day_abnormal:
    @classmethod
    def setup_class(cls):
        pass
    ...
    def test_statistics_day_abnormal(self):
        ...
        if reportIds != []:
            for reportId in reportIds:
                ...
                url = "/ecg/statistics_day_abnormal?report_id=" + str(reportId[1])
                result = Request.execute("config_reviewDr.xlsx", "ecg", "statistics_day_abnormal", url)
                log.info("返回结果为：")
                log.info(result)
