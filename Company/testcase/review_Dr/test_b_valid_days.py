    ...
    def test_valid_days(self):
        ...
        if reportIds != []:
            for reportId in reportIds:
                ...
                result = Request.execute("config_reviewDr.xlsx", "ecg", "valid_days", url)
                log.info("返回结果为：")
                log.info(result)
        ...
        path1 = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) + "/configure/Review_valid_days.yml"
        if 'Windows' in platform.system():
            # windows os
            path1 = path1.replace('/', '\\')
